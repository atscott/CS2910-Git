...
public class UI extends JFrame {
	...
	public UI() {
		//This gets the current screen width and height and centers the GUI on the users screen
	...
	public void initTextFieldsAndButtons() {
		...
		btnStartRequest.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent event) {
				if (!lblPortWarning.isVisible() && !lblHostNameWarning.isVisible() && txtPortNumber.getText().length() > 0 && txtHostName.getText().length() > 0) {
					if (!UI.this.validateHostName() && !UI.this.validatePortNumber()) {
						HttpClient.startRequest(txtHostName.getText(), Integer.parseInt(txtPortNumber.getText()));
					}
				}
			}
			@Override
		...
		lblHostNameWarning = new JLabel("The host name must be a valid ip address or of form http://www.example.com");
