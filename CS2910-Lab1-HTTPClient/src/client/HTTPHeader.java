...
public class HTTPHeader {
	...
	/**
	 * if the chunkedEncoding is false, this is the length of the content to be read
	 * if the chunkedEncoding is true, this will be negative
	 */
	int contentLength;
	...
	public boolean setContentType(String contentType) {
		...
		for (int i=0;i<Constants.MimeType.values().length;i++) {
			System.out.println(Constants.MimeType.values()[i].contentType);
			if (Constants.MimeType.values()[i].contentType.equals(contentType)) {
				validType=true;
				System.out.println("valid MimeType found, assigning "+Constants.MimeType.values()[i].contentType);
